...
V7 曾把 q/r 的读取改成有延迟的 4-bit distance-gossip，却仍允许 source 瞬时修改远端 express 入口的 reservation，因此物理叙事并不闭合。V8 新增的 `registered` 模式把每个有向 express 入口设为其 q/r 唯一 authority：source 选定路线后只增加本地 pending，route-setup record 沿 committed path 传播，到达入口才执行 `r++`；ACK 与包含该登记的延迟广告对齐返回，packet 穿过入口时本地 `r--`，转入 escape 时 cancel 也必须逐跳传播。其他 router 只看到按 `1 + Manhattan distance` 延迟的 4-bit 广告以及自己的未确认 pending，不能瞬时读写全局状态。最终权重相应调整为 `static + 1.0q + 0.6r`，不使用随机 admission。control record 当前在独立 sideband 上建模传播时间但不与 data VC 竞争，因此协议时序是物理闭合的，控制 overlay 的面积、能耗和排队仍是明确限制，而非零成本假设。完整设计和反例见 `PHYSICAL_EXPRESS_INFO_V8_RESULTS_ZH.md`。
...
V8 对“非瞬时全局 express 状态”又做了一轮独立验证。固定 top-8 committed routing、XY mesh segment、escape 和 `distance-gossip/period 1/base delay 1/4-bit/registered`，只用 `static + 1.0q + 0.6r` 评分，在 Garnet rate 0.8、20k+100k 下，Uniform 的 Mesh、20-layout Random expectation、traffic-aware Greedy、SA 依次为 `0.266210/17886`、`0.283949/16296`、`0.323386/12235`、`0.330633/10570`；Tornado 相应为 `0.211563/23719`、`0.225865/21283`、`0.368482/3769`、`0.369664/3349`。Tornado 的旧 SA 在 registered 语义下会低于 Greedy，因此没有通过调 routing 掩盖问题，而是固定全部 routing 参数重新做 parallel-tempering placement search。新 placement 又在未参与筛选的 seeds 4--6 长测得到 Greedy `0.368373/3797`、SA `0.369757/3358`；合并 6 seeds 后 SA 吞吐提高 0.35%、延迟降低 11.37%。这使两类 traffic 的观测均值都满足 `Mesh < Random expectation < traffic-aware Greedy < SA Pareto`，但 SA 的吞吐领先很小，应该报告原始数值和 holdout，而不能宣称普遍或大幅领先。所有正式 registered run 的 late registration 为 0；控制记录约为 0.75--1.01 个每 delivered packet，q/r 广告还会产生较高 multicast 负载，所以后续仍需显式建模 control overlay 带宽。把广告周期增至 2 的低带宽折中也通过了 selected-placement 长测：Uniform Greedy/SA 为 `0.321364/12659`、`0.328763/10854`，Tornado 6-seed 为 `0.369595/3706`、`0.370324/3337`，standalone 广告接收事件约下降 25%--27%；但 P2 的 20 Random × 3 seeds Garnet 正式矩阵尚未补齐，因此完整 hierarchy 仍以 P1 为主证据。
